...
export function applyCounselingStateDelta({
  ...
  const normalizedUserMessages = safeArray(userMessages)
  ...
  const allowedUserMessageIds = new Set(
    normalizedUserMessages.map((item) => item.id)
  );
  allowedUserMessageIds.add(safeMessageId);

  const userMessageTextById = new Map(
    normalizedUserMessages.map((item) => [item.id, item.text])
  );
  ...
  const reject = (collection, index, reason) => {
    rejected.push({ collection, index, reason });
  };

  for (const collection of Object.keys(COLLECTION_PREFIX)) {
    pending[collection] = [];
    const rawItems = safeArray(delta[collection]);
    providedItemCount += rawItems.length;

    for (const [index, raw] of rawItems.entries()) {
      if (!raw || typeof raw !== 'object' || Array.isArray(raw)) {
        reject(collection, index, `invalid_${collection}`);
        continue;
      }
      ...
      if (collection === 'subjects') {
        ...
        if (!label) {
          reject(collection, index, 'invalid_subject');
          continue;
        }
        const subject = {
      ...
      let text = cleanText(raw.text, 800);
      let recoveredFromSource = false;

      if (!text && sourceValidation.ok) {
        const sourceText = sourceValidation.sources
          .map((id) => userMessageTextById.get(id) || '')
          .filter(Boolean)
          .join(' ');
        text = cleanText(sourceText, 800);
        recoveredFromSource = Boolean(text);
      }

      if (!text) {
        reject(collection, index, `missing_${collection}_text`);
        continue;
      }
      if (!sourceValidation.ok) {
        reject(collection, index, sourceValidation.reason);
        continue;
      }
      ...
      if (subjectId) {
        ...
        if (!subjectExists) {
          reject(collection, index, 'subject_not_found');
          continue;
        }
      ...
      else if (recoveredFromSource) item.kind = 'user_statement_fallback';
      ...
      if (collection === 'corrections') {
        ...
        if (!target) {
          reject(collection, index, 'correction_target_not_found');
          continue;
        }
  ...
  if (providedItemCount > 0 && acceptedItemCount === 0) {
    return {
      state: current,
      applied: false,
      status: 'state_update_skipped',
      reason: rejected[0]?.reason || 'no_valid_state_items',
      rejected
    };
  }
  ...
  return {
    state: next,
    applied: true,
    status: rejected.length ? 'state_updated_partial' : 'state_updated',
    reason: rejected.length ? 'partial_invalid_items' : null,
    rejected
  };
}
