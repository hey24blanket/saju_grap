...
export function compactRelationMembers(relation) {
  ...
  return relation.members
    ...
    .filter((member) => member && (member.position || member.value))
    .slice(0, 6);
}

export function compactRelation(relation) {
  if (!relation || typeof relation !== 'object') return null;
  const members = compactRelationMembers(relation);
  return {
    type: relation.relationType ?? null,
    complete: relation.complete ?? null,
    // Preserve which chart positions the relation binds (e.g. cycle_branch ↔
    // natal_day_branch). Engine already computed these members; we only carry
    // them into counseling evidence instead of dropping positional context.
    members,
    ...(relation.punishmentType
      ? { punishmentType: relation.punishmentType }
      : {}),
...
function summarizeProjection(projection, domainKey, { fromIndex = 0 } = {}) {
  const labels = Array.isArray(projection?.labels) ? projection.labels : [];
  const values = Array.isArray(projection?.[domainKey]) ? projection[domainKey] : [];
  const entries = labels
    ...
    .filter((item) => item.index >= fromIndex && Number.isFinite(item.value));
  if (!entries.length) return null;

  const maximum = Math.max(...entries.map((item) => item.value));
  const minimum = Math.min(...entries.map((item) => item.value));
  return {
    source: projection.source,
    canonicalEngineFact: false,
    flat: maximum === minimum,
    relativelyStronger: entries
      .filter((item) => item.value === maximum)
      .slice(0, 4)
      .map(({ label, value }) => ({ label, value })),
    relativelyPressured: entries
...
export function buildCounselingFactContext({
  ...
  const annualStartIndex = intent.wantsPast && !intent.wantsFuture
    ? 0
    : Math.max(0, annual.findIndex((cycle) => cycle.year >= referenceYear));
  const relativeWindows = {
    source: 'ui_compatibility_wave_projection',
    canonicalEngineFact: false,
    warning: '질문에 직접 답하기 위한 상대 비교 보조값이다. 실제 사건이나 수입을 보장하지 않는다.',
    selectedDomainKey: projectionDomain,
    annual: summarizeProjection(annualProjection, projectionDomain, {
      fromIndex: annualStartIndex
    }),
    monthly: monthly.map((item) => ({
      year: item.year,
      all: summarizeProjection(item.projection, projectionDomain),
      fromReferenceMonth: item.year === referenceYear
        ? summarizeProjection(item.projection, projectionDomain, {
            fromIndex: Math.max(0, (numberOrNull(reference.month) || 1) - 1)
          })
        : summarizeProjection(item.projection, projectionDomain)
    }))
  };
