...
function extractTargetMonth(text) {
  return extractTargetMonths(text)[0] ?? null;
}
...
export function buildCounselingConversationFocus({
  ...
  const followupMode = detectFollowupMode(currentText);
  const coverageIntent = isOverallFortuneAsk(currentText) ? 'overall' : null;
  ...
  if (followupMode === 'explain_reason' && task === 'general') {
    task = 'explanation';
  }

  if (followupMode === 'broaden' || coverageIntent === 'overall') {
    if (!currentDomainExplicit) domain = 'all';
  }

  const priorText = priorUsers.map((item) => item.text).join('\n');
  const currentYears = extractTargetYears(currentText, referenceYear);
  const inheritedYears = extractTargetYears(priorText, referenceYear);
  const currentMonths = extractTargetMonths(currentText);
  const inheritedMonths = extractTargetMonths(priorText);
  const targetYears = (currentYears.length ? currentYears : inheritedYears);
  const targetMonths = (currentMonths.length ? currentMonths : inheritedMonths);

  if (currentYears.length === 0 && inheritedYears.length > 0 && priorUsers.length) {
    inherited = inherited || !currentDomainExplicit;
    sourceUserMessageIds = [
      ...priorUsers.map((item) => item.id).filter(Boolean),
      ...(messageId ? [messageId] : [])
    ];
  ...
  return {
    ...
    inherited,
    followupMode,
    coverageIntent: coverageIntent || (followupMode === 'broaden' ? 'overall' : null),
    sourceUserMessageIds: [...new Set(sourceUserMessageIds.filter(Boolean))],
    targetYears,
    targetMonths,
    targetMonth: targetMonths[0] ?? extractTargetMonth(currentText)
  };
}
