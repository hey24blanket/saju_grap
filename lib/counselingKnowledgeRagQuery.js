...
function evidenceKeywords(evidencePacket) {
  ...
  for (const item of evidencePacket?.evidence || []) {
    ...
    if (Array.isArray(facts.relevantGroups)) {
      facts.relevantGroups.slice(0, 5).forEach((group) => {
        if (group?.group) parts.push(`natalGroup:${group.group}`);
        if (group?.strengthBand) parts.push(`natalGroupBand:${group.group || 'unknown'}:${group.strengthBand}`);
      });
    }
    if (Array.isArray(facts.tenGodPositions)) {
      facts.tenGodPositions.slice(0, 6).forEach((item) => {
        if (item?.tenGodKo) parts.push(`natalTenGod:${item.tenGodKo}`);
        if (item?.group) parts.push(`natalTenGodGroup:${item.group}`);
        if (item?.position) parts.push(`natalPosition:${item.position}`);
      });
    }
    if (Array.isArray(facts.natalRelations)) {
      facts.natalRelations.slice(0, 4).forEach((rel) => {
        if (rel?.type) parts.push(`natalRelation:${rel.type}`);
      });
    }
    if (Array.isArray(facts.detectedStars)) {
      facts.detectedStars.slice(0, 4).forEach((star) => {
        if (star?.starId) parts.push(`star:${star.starId}`);
      });
    }
  }
  return [...new Set(parts)].slice(0, 36);
}
