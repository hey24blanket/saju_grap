...
export function buildCounselingOrchestratorDiagnostic({
  ...
  return {
    focus: {
      ...
      followupMode: focus?.followupMode || null
    },
    ...
    issueOverlap: issueOverlap
      ? {
          mode: issueOverlap.mode || 'simple',
          strandCount: Array.isArray(issueOverlap.strands)
            ? issueOverlap.strands.length
            : 0,
          strands: (issueOverlap.strands || []).map((strand) => ({
            kind: strand.kind,
            summary: strand.summary,
            grounding: strand.grounding
          }))
        }
      : null,
    interpretationBrief: interpretationBrief
      ? summarizeInterpretationBrief(interpretationBrief)
      : null,
    interpretationCommitment: interpretationBrief?.interpretationCommitment ?? null,
    followupMode: interpretationBrief?.followupMode || focus?.followupMode || null,
    answerDirectness: interpretationBrief?.answerDirectness || null,
    requestedPeriodCoverage: interpretationBrief?.requestedPeriodCoverage || null,
    supportedMeaningCount: Array.isArray(interpretationBrief?.supportedMeanings)
      ? interpretationBrief.supportedMeanings.length
      : 0,
    interpretationBriefRaw:
      includeRawInterpretationBrief && interpretationBrief
        ? interpretationBrief
        : undefined
  };
}
...
export function buildCounselingOrchestration({
  ...
  const issueOverlap = buildIssueOverlap({
    userMessage,
    focus,
    evidencePacket
  });

  const interpretationBrief = buildCounselingInterpretationBrief({
    userMessage,
    focus,
    evidencePacket
  });
...
export function finalizeCounselingOrchestrationAfterRag({
  orchestration,
  userMessage = '',
  knowledgeHits = [],
  ...
  return {
    ...orchestration,
    interpretationBrief,
    interpretationBriefText: formatInterpretationBriefForPrompt(interpretationBrief)
  };
}
