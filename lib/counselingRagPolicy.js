...
function hasExplicitCanonicalDomainFocus(focus) {
  return Boolean(
    focus?.domain &&
      focus.domain !== 'all' &&
      focus.inherited !== true
  );
}

function shouldRetrieveForExplicitDomainGeneral(focus, evidencePacket) {
  if (focus?.task !== 'general') return false;
  if (!hasExplicitCanonicalDomainFocus(focus)) return false;
...
export function shouldRetrieveKnowledgeForFocus({
  ...
  if (shouldRetrieveForExplicitDomainGeneral(focus, evidencePacket)) {
    return true;
  }
...
export function shouldRetrieveExampleStrategyForFocus({
  ...
  if (
    task === 'general' &&
    hasExplicitCanonicalDomainFocus(focus)
  ) {
    return true;
  }
