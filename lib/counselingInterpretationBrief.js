  ...
  wealth: [
    {
      id: 'wealth_timing',
      kind: 'timing_broad',
      pattern: /(언제|시기|풀리|풀릴|나아|자금|현금|돈이)/u
    },
    {
      id: 'wealth_reason',
      kind: 'explanation',
...
function buildAnswerPlan({ coverage, evidencePacket, focus }) {
  const hasCycle = hasScope(evidencePacket, ['daewoon', 'year', 'month']);
...
export function buildCounselingInterpretationBrief({
  ...
  const answerPlan = buildAnswerPlan({ coverage, evidencePacket, focus });
  ...
  return {
    ...
    selectedEvidence,
    // Meaning is produced by the model, not fabricated by the server-side
    // Brief. This stays empty so the Brief never asserts unsupported meaning.
    supportedMeanings: [],
    insufficientAreas,
    forbiddenInferences,
    answerPlan
