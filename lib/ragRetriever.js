...
export function isRagDocumentRetrievable(
  document
) {
  if (
    !document ||
  ...
  // Explicit denial always wins over legacy compatibility or positive flags.
  if (
    document.status ===
      'inactive' ||
    document.retrievalAllowed ===
      false ||
    document.isActive ===
      false ||
    reviewed.status ===
      'inactive' ||
    reviewed.retrievalAllowed ===
      false ||
    reviewed.isActive ===
      false ||
    reviewed.isNegative ===
  ...
  // Preserve the existing legacy policy: missing activation fields are allowed,
  // but an explicit non-active status is not.
  if (
    document.status &&
    document.status !==
      'active'
