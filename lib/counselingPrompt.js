export const COUNSELING_PROMPT_VERSION =
  'sajugrap_counseling_prompt_v4_orchestrator';
...
export function buildCounselingTurnPrompt({
  ...
  const turnRequirements = [
    asksFutureTiming || focusTask === 'timing'
      ? timingGrounded === true
        ? '- 시기 질문입니다. RELEVANT SAJU EVIDENCE의 highlight month evidence와 KNOWLEDGE RAG 해석만 연결해 구체적 월을 설명하세요. relativeWindowHint는 후보 선정용이며 사용자에게 단독 근거로 말하지 마세요.'
        : timingYearGrounded === true
          ? '- 시기 질문입니다. 월 단위 canonical 근거가 부족하므로 특정 월을 단정하지 마세요. RELEVANT SAJU EVIDENCE의 highlight year / daewoon / natal Fact와 KNOWLEDGE RAG로 연·대운 수준에서 설명하세요.'
          : '- 시기 질문입니다. 선택된 canonical cycle evidence만으로는 구체적 연·월 단위 근거가 부족합니다. relativeWindows나 파동 점수만으로 기간을 단정하지 말고, 대운·원국 구조와 현재 자료 한계를 짧게 밝히세요.'
      : '',
    focusTask === 'explanation' || (focusInherited && /(왜|이유|명리)/u.test(safeMessage))
      ? '- 명리 설명 follow-up입니다. assistant history의 시기·결론을 새 사실처럼 복사하지 말고, RELEVANT SAJU EVIDENCE와 KNOWLEDGE RAG로 같은 주제의 근거를 다시 구성하세요. 근거 없이 직전 답을 부정하지 마세요.'
      : '',
    focusTask === 'reality_bridge'
      ? '- 현실 확인 질문입니다. RELEVANT SAJU EVIDENCE의 실제 Fact에서 출발해 명리 구조를 일상어로 번역한 뒤, 그 해석을 검증할 현실 확인점을 제안하세요. 일반 자기계발 체크리스트만으로 답하지 마세요. 사용자가 말하지 않은 정산·미수금·부채·지출 문제를 사실처럼 만들지 마세요.'
      : '',
    asksForChoices
