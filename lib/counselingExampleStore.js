...
export async function searchCounselingExamples({
  ...
  for (const doc of snapshot.docs) {
    ...
    if (result.schemaVersion === COUNSELING_EXAMPLE_SCHEMA_VERSION_V2) v2.push(result);
    else v1.push(result);
  }

  const preferred = v2.length ? v2 : v1;
  return {
    query: text,
    category: wantedCategory || null,
    includeInactive: Boolean(includeInactive),
    preferredSchema: v2.length ? COUNSELING_EXAMPLE_SCHEMA_VERSION_V2 : COUNSELING_EXAMPLE_SCHEMA_VERSION,
    results: preferred.slice(0, targetLimit)
  };
}
