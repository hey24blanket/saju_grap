...
const MAX_BATCH = 32;
const MAX_VECTOR_CANDIDATES = 600;

function resolveVectorCandidateLimit({
  targetLimit,
  includeInactive,
  v2Only
}) {
  if (includeInactive) {
    return MAX_VECTOR_CANDIDATES;
  }
  ...
  return Math.min(120, Math.max(targetLimit * 30, targetLimit));
}

export function filterCounselingExampleVectorCandidates(
  candidates,
  {
    wantedCanonical = null,
    v2Only = false,
  ...
  for (const candidate of candidates) {
    ...
    if (
      !includeInactive &&
      (data.status !== 'active' || data.retrievalAllowed !== true)
    ) {
      continue;
    }
    ...
    const result = resultFromDocument({
      id: candidate.id,
      data: () => ({
        ...data,
        __distance: candidate.vectorDistance
      })
    });
  ...
  return pickCounselingExampleSearchResults({
    v2,
    v1,
    v2Only,
    targetLimit
...
export async function searchCounselingExamples({
  ...
  const wantedCanonical = resolveV2ExampleSearchCategory(category);
  ...
  const db = await checkedFirestore();
  const candidateLimit = resolveVectorCandidateLimit({
    targetLimit,
    includeInactive,
    v2Only
  });
  ...
  const candidates = snapshot.docs.map((doc) => {
    const data = doc.data();
    const distance = Number(data.__distance);
    return {
      id: doc.id,
      data,
      vectorDistance: Number.isFinite(distance) ? distance : null
    };
  });

  const preferred = filterCounselingExampleVectorCandidates(candidates, {
    wantedCanonical,
    v2Only,
    includeInactive,
    targetLimit
  ...
  return {
    ...
    stats: {
      ...
      vectorCandidatesReturned: candidates.length
    }
  };
}

function buildV2Context(item, index, { strategyOnly = true } = {}) {
  const strategy = safeObject(item.strategy);
  const guidance = safeArray(strategy.turnGuidance);
  const arc = safeObject(strategy.counselingArc);
  ...
  const arcSummary = [
    arc.start ? `start=${cleanText(arc.start, 400)}` : '',
    arc.development ? `development=${cleanText(arc.development, 400)}` : '',
    arc.correction ? `correction=${cleanText(arc.correction, 400)}` : '',
    arc.integration ? `integration=${cleanText(arc.integration, 400)}` : ''
  ].filter(Boolean).join('; ');

  const strategyBlock = [
    `[COUNSELING STRATEGY EXAMPLE ${index + 1}]`,
    `domain=${cleanText(item.domain || item.category, 80)}`,
    `scenario=${cleanText(item.scenarioTitle, 300)}`,
    `arc_type=${cleanText(item.arcType, 120)}`,
    `plot_phase=${safeArray(item.plotPhases).join(' -> ')}`,
    `counseling_goal=${cleanText(strategy.counselingGoal, 800)}`,
    arcSummary ? `counseling_arc: ${arcSummary}` : '',
    'TURN GUIDANCE:',
    guidanceText,
    'Strategy only. Do not copy example dialogue, golden answers, Engine Facts, dates, or real-world facts into the current user.'
  ].filter(Boolean);

  if (strategyOnly) {
    return strategyBlock.join('\n');
  }
  ...
  return [
    ...strategyBlock,
    'EXAMPLE DIALOGUE (reference only — do not copy into live reply):',
    dialogue
  ].join('\n');
}

export function buildCounselingExampleContext(
  results,
  { maxExamples = 3, strategyOnly = true } = {}
) {
  return safeArray(results).slice(0, maxExamples).map((item, index) => {
    if (item?.schemaVersion === COUNSELING_EXAMPLE_SCHEMA_VERSION_V2) {
      return buildV2Context(item, index, { strategyOnly });
    }
