...
const RAG_TARGET_RESULTS =
  Number(
    process.env.RAG_TARGET_RESULTS ||
    6
  );

const RAG_CANDIDATE_LIMIT =
  Number(
    process.env.RAG_CANDIDATE_LIMIT ||
    100
  );

const RAG_MAX_CHARS_PER_CHUNK =
  Number(
    process.env.RAG_MAX_CHARS_PER_CHUNK ||
    3500
  );
...
function buildRagUserQuery(
  ...
  if (
    ...
    return (
      `${normalized.cycle}의 총운, 사업운, 재물운, 심신운, 연애운을 Engine Facts에 맞춰 행동 전략으로 해석하는 기준`
    );
  }
  ...
  if (
    ...
    return (
      `${normalized.role}의 의미, 현재 구조에서의 작동 조건, 행동 전략, 금지 해석`
    );
  }
  ...
  if (
    ...
    return (
      `${normalized.cycle} ${normalized.domain}의 현재 구조와 행동 전략`
    );
  }
  ...
  if (
    ...
    return (
      `${normalized.cycle} ${normalized.domain || normalized.role || '총운'}의 심층 전략, 기회, 마찰 요인, 실행 계획`
    );
  }
...
export default async function handler(
  ...
  // ---------------------------------------------------------------------------
  // Scope Guard
  // 명백한 범용 업무 대행 요청은 Engine/RAG/LLM 호출 전에 종료한다.
  // ---------------------------------------------------------------------------
