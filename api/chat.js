// api/chat.js
// 300자 3단 구조화 & 백그라운드 5대 운 일괄 프리페치 지원 API
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 모드 1: 백그라운드 5대 운 일괄 프리페치 (JSON Schema)
    if (mode === 'prefetch') {
      isJsonMode = true;
      maxTokens = 2000;
      userPrompt = `
...
각 영역별 현재 파동 점수:
- 총운: ${cycleScores?.all ?? 0}점
- 사업운: ${cycleScores?.career ?? 0}점
- 재물운: ${cycleScores?.wealth ?? 0}점
- 심신운: ${cycleScores?.mental ?? 0}점
- 연애운: ${cycleScores?.love ?? 0}점

[작성 규칙]
반드시 [3단 문장 규격: 상태 진단 -> 흐름 분석 -> 구체적 실행 과제]에 맞춰 각 항목당 공백 포함 280~320자 내외로 풍부하게 작성하세요. 다른 영역과 겹치지 않는 고유한 내용을 서술하십시오.
...
`;
    ...
    // 모드 2: 단일 300자 생성
      ...
      if (role) {
        ...
        userPrompt = `내담자 ${userName}님(일주: ${dayHanja}, ${dayPillar}일주)의 [${cycle} 주기 - ${domain}] (파동 에너지 점수: ${score >= 0 ? '+' : ''}${score}점)에 대한 맞춤 행동 전략을 [3단 문장 규격: 상태 진단 1문장 -> 흐름 분석 1~2문장 -> 구체적 행동 제안 1~2문장]에 맞춰 공백 포함 정확히 280~320자 분량으로 작성해 주세요.`;
      }
    } 
    // 모드 3: 1,500자 상세 리포트
    else if (mode === 'detail') {
